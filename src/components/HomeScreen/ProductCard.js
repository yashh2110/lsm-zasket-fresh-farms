...
const ProductCard = ({
  ...
  return (
    <View style={{ flex: 1, margin: 4, width: 160, marginBottom: 20 }}>
      ...
      <TouchableOpacity
        onPress={() => {
          navigation.navigate("ProductDetailScreen", { item: item?.id });
        }}
      ...
      >
        ...
        >
          <Image
            ...
            resizeMode="contain"
            source={
              item?.itemImages[0]?.mediumImagePath
                ? { uri: item?.itemImages[0]?.mediumImagePath }
        ...
        <View style={[{ padding: 10 }]}>
          ...
          <Text
            numberOfLines={1}
            style={{ fontSize: 12, color: "#909090", marginVertical: 5 }}
          >
            {item?.itemSubName}{" "}
          </Text>
        </View>
      ...
      {item?.onDemand == false &&
      item?.availableQuantity < 1 ? null : !loadingCount ? (
        addButton ? (
          <TouchableOpacity
            onPress={() => onAddToCart()}
