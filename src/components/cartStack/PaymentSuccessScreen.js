...
const PaymentSuccessScreen = ({
  ...
  const { date, slotTime, firebaseEventParams, afEventParams } = route.params;
  ...
  useEffect(() => {
    const count = async () => {
      await getOrderCount((res, status) => {
        if (status) {
          if (res == 1) {
            ...
            const afEventName = "af_purchase";
            const FirebaseEventName = "purchase";
            appsFlyer.logEvent(
              afEventName,
              afEventParams,
              (res) => {
    ...
    if (firebaseEventParams && afEventParams) {
      count();
    } else {
      console.error("event params are not available");
    }
  }, []);
  useEffect(() => {
    // alert(slotTime)
      ...
      <View style={{ backgroundColor: "white", alignItems: "center", flex: 1 }}>
        ...
        <Text style={{ fontSize: 14, fontWeight: "bold" }}>
          {moment()
            ...
            .format("DD MMM")}{" "}
          ({" "}
          <Text style={{ fontSize: 14, fontWeight: "bold" }}>0{slotTime}</Text>{" "}
          )
        </Text>
...
export default connect(mapStateToProps, { getLeaderBoardList, getOrderCount })(
