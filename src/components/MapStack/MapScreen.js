...
class MyMapView extends React.Component {
    ...
    if (fromScreen == "EDIT_SCREEN" && regionalPositions == null) {
      ...
      await this.setState({
        ...
        alternateMobileNumber: item?.alternateMobileNumber
          ? item?.alternateMobileNumber.replace("+91", "")
          : "",
      });
    } else {
      ...
      await this.setState({
        ...
        mobileNumber: parsedUserDetails?.customerDetails?.userMobileNumber.replace(
          "+91",
          ""
        ),
      });
  ...
  validate = () => {
    ...
    if (this.state.houseNumber == undefined || this.state.houseNumber == "") {
      this.setState({
        houseNumberErrorText:
          "House No / Flat No / Floor /Building is required",
      });
      status = false;
    }
  ...
  onSubmit = async () => {
    if (this.validate()) {
      ...
      payload = {
        ...
        alternateMobileNumber: this.state.alternateMobileNumber.includes("+91")
        ...
        await this.props.updateUserAddress(
          ...
          async (response, status) => {
            if (status) {
              ...
              if (
                response?.data ==
                "Cannot update address to not serviceable area"
              ) {
          ...
          async (response, status) => {
            if (status) {
              ...
              if (this.state.mode === "ON_INITIAL") {
                this.props.navigation.navigate("SetAuthContext", {
                  userLocation: location,
                }); // if you send it as null it wont navigate
      ...
      await this.props.updateUserAddress(
        ...
        async (response, status) => {
          if (status) {
            ...
            this.setState({ errorMessage: response?.data });
            if (
              response?.data == "Cannot update address to not serviceable area"
            ) {
              await this.setState({
                mode: "AddNew_SCREEN",
                scrollEnable: true,
                  ...
                  <TouchableOpacity
                    ...
                    <View
                      ...
                      <Icon
                        ...
                        style={{ fontSize: 24, color: "#232323" }}
                      />
                    </View>
                    <View style={{ flex: 1, justifyContent: "center" }}>
