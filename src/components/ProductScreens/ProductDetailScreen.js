...
const ProductDetailScreen = ({
  ...
  const initialFunction = () => {
    getItem(route?.params?.item, async (res, status) => {
      if (status) {
        ...
        analytics().logEvent("content_view", {
          price: response?.actualPrice,
          content: response?.itemName,
          content_id: response?.id,
          content_type: response?.categoryName,
          currency: "INR",
        });
        setLoading(false);
        setRefresh(false);
  ...
  useEffect(() => {
    let filteredItems = cartItems.filter(
      (element) => element?.id == route?.params?.item
    );
    ...
    >
      ...
      >
        ...
        >
          ...
          <View style={{}}>
            ...
            >
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => {
                    moreShare(
                      item?.id,
                      item?.itemImages?.[0]?.largeImagePath,
                      item?.shareInfo?.message
                    );
                  }}
              ...
              >
                ...
                {item?.discountedPrice == item?.actualPrice ? undefined : (
                  <Text
                    style={{
                      fontSize: 14,
                      color: "#909090",
              ...
              <Text style={{ color: "#909090" }}>{item?.itemSubName} </Text>
              <Text style={{ marginTop: 5, color: "#727272" }}>
          ...
          {item?.onDemand == false &&
          item?.availableQuantity < 1 ? null : addButton ? (
            <TouchableOpacity
              onPress={() => onAddToCart()}
              style={{
            ...
            >
              <TouchableOpacity
                onPress={() => onCartUpdate("DECREASE")}
                style={{
                  width: 35,
                  height: 35,
                  backgroundColor: "#B90E14",
                  borderRadius: 4,
                  justifyContent: "center",
                  alignSelf: "center",
                }}
...
export default connect(mapStateToProps, { getItem, updateCartItemsApi })(
  ProductDetailScreen
);
